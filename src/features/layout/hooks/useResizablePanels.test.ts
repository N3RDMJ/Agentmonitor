...
describe("useResizablePanels", () => {
  ...
  it("reads stored sizes and clamps to bounds", () => {
    window.localStorage.setItem("geminimonitor.sidebarWidth", "999");
    window.localStorage.setItem("geminimonitor.rightPanelWidth", "100");
    window.localStorage.setItem("geminimonitor.planPanelHeight", "not-a-number");
  ...
  it("persists sidebar width changes and clamps max", () => {
    ...
    expect(hook.result.sidebarWidth).toBe(420);
    expect(window.localStorage.getItem("geminimonitor.sidebarWidth")).toBe(
      "420",
    );
